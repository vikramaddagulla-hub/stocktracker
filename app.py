...
# Fallback text box if key is not configured in secrets
if not gemini_key:
...
timeframe = st.sidebar.select_slider("Chart Period", options=["1d", "5d", "1mo", "6mo", "1y", "5y", "max"], value="1y")
...
try:
    with st.spinner(f"Loading live market data for {user_ticker}..."):
        info, hist, news_data = load_stock_data(user_ticker)
except Exception:
...
with tab_chart:
    if not hist.empty:
        ...
        fig.add_trace(go.Scatter(
            x=chart_data.index, y=chart_data['SMA50'], mode='lines', name='50 SMA', line=dict(color='#E3B341', width=1.5)
        ))
    ...
    if not gemini_key:
        ...
        try:
            ...
            if prompt := st.chat_input(f"Ask Gemini about {company_name}'s valuation, risks, or financial health..."):
                ...
                # Construct rich financial system prompt
